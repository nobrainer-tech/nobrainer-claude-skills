# Exact judge input — final verified routing holdout v1.2.0

Assembled at 2026-08-28T18:08:46Z after the candidate completed, from the
...
<candidate_prompt>
# Frozen final verified routing holdout — v1.2.0

Frozen before its first run at 2026-08-28T18:05:19Z.
...
## Case A — superseded ready work has not been sent

`CSV-4` is `READY` under fingerprint `plan-old`; `GUIDE-4` is `PENDING`
and depends on it. The owner explicitly replaces CSV with Parquet before
dispatch. No prompt, lease or implementation exists. Apply the owner-decision
hook and state both task states, evidence treatment and the first condition that
can permit readiness again.

## Case B — superseded work may already be active

`DEPLOY-DOC` is `SENT` to an exact known session when the owner changes the
required target. Delivery was read back, but current execution and lease state
are unknown. A dependent task is still `PENDING`. State what can be changed
immediately, who owns the stop request, and what must not be claimed without
readback.

## Case C — review repair is not fully closed

`AUTHZ-8` is `CORRECTION_REQUIRED` after a required review found a bypass.
Build repaired it and affected tests passed. The required review has not rerun,
and therefore no post-review receive-audit exists. `CLIENT-8` depends on it.
Run Dispatcher `RECONCILE`; do not implement or send work.

## Case D — transport submission is not delivery

`INDEX-5` is `READY`. Sessions verifies exact identity, checkout, free lease
and no active turn, then submits one prompt. The API returns success locally but
no target readback or message ID can be obtained. State the scheduler transition
and retry boundary.

## Case E — current external evidence is mandatory

A package-install failure is ambiguous: a relevant wiki page records an old
workaround, current local logs disagree with it, and the correct remedy depends
on the latest package-manager behavior. Internet research is unavailable. Run
the problem gate and state whether the workaround may be applied.
...
<contract path="skills/nobrainer-ultra/references/correction-hooks.md">
...
A changed owner decision supersedes the old requirement, moves affected
not-started `READY` work to `STOPPED`, keeps dependants blocked and requires a
new plan fingerprint before readiness. A failed review returns to Build and
