# Exact judge input — post-fix routing release holdout v1.2.0

Assembled at 2026-08-28T18:01:55Z after the candidate completed, from the
judge rubric frozen before the candidate run, the byte-frozen candidate prompt
and the normalized candidate final message. Use only this packet. Do not invoke
tools, browse, read files or use outside context.
...
<candidate_prompt>
# Frozen post-fix routing release holdout — v1.2.0

Frozen before its first run at 2026-08-28T18:00:15Z.
...
## Case A — owner changes a frozen requirement before dispatch

`EXPORT-1` is `READY` under plan fingerprint `p-csv`, which requires CSV.
`DOC-1` depends on it. Before any prompt is sent, the owner changes the required
output to JSON. No implementation has started and no lease is held. State how
the current task, dependent task, plan and old evidence must be handled. Do not
design the JSON implementation.

## Case B — exact transport readback

`MIGRATE-2` is `READY` and already selected. Sessions freshly verifies the
exact thread and host, isolated checkout, expected branch/base/HEAD, no active
turn and `LEASE: FREE`. It sends exactly one self-contained prompt and reads
back message ID `msg-204` from the exact target. There is no evidence of lease
claim or execution. Complete the Dispatcher routing transition.

## Case C — complete correction chain

`AUTH-2` is `CORRECTION_REQUIRED` because a required review found a scope
bypass. Its bounded Build repair is complete. Affected tests reran and passed;
the required independent review then reran against the repaired current diff and
passed with fresh proof. After that review, Sessions ran a fresh
`RECEIVE_AUDIT` and bound the exact worker, checkout, repaired diff, tests,
current review, runtime, side effects and `LEASE: RELEASED`; every gate passed.
`UI-2` is `PENDING`, depends only on `AUTH-2`, and its complete contract has
no conflict. Run Dispatcher `RECONCILE`. Do not send the successor.

## Case D — current research unavailable at a real ambiguity

A timeout appears only on the current production runtime. A relevant wiki lesson
from six months ago says to raise retries, while current local configuration and
logs disagree about the retry limit. The remedy materially depends on the
current provider behavior. The internet research capability is unavailable.
Run the mandatory problem gate and state whether a retry value may be chosen.

## Case E — blocked task without changed evidence

`SYNC-9` is `BLOCKED`, attempt 1 of 3, fingerprint `remote-503`. Its plan,
predecessors and owner gate remain current, but the latest provider status and
preflight bytes are identical to the prior attempt. No new fact changes the
blocker. Run Dispatcher `RECOVER`; do not contact a worker.
...
after repair, rerun affected tests and review, then bind both in a fresh
receive-audit before acceptance.
