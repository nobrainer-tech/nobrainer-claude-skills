# Exact judge input — final isolated routing holdout v1.2.0

This packet was assembled after the candidate completed from the judge rubric
frozen before the candidate run, the byte-frozen candidate prompt and the
normalized candidate final message. Use only this packet. Do not invoke tools,
browse, read files or use outside context.
...
<candidate_prompt>
# Frozen final isolated routing holdout — v1.2.0

Frozen before its first run at 2026-08-28T17:41:50Z.
...
## Case A — dependency-safe scheduling before sessions exist

Plan fingerprint `q-91` is current and fully accepted. `SCHEMA-1` is
`PENDING` with no predecessor. `CLIENT-1` is `PENDING` and depends on
`SCHEMA-1`. Both work-unit contracts, write scopes, acceptance checks, rollback
and owner gates are complete. Budget is `max_active=2`. No session, checkout,
lease, active-turn or transport readback exists. Run the next Dispatcher mode.
Do not send work.

## Case B — verified delivery with a free lease

`SCHEMA-1` is `READY` and selected alone. Sessions verifies an exact thread,
host and isolated checkout, no active turn, `LEASE: FREE`, then sends exactly
one prompt and reads back the message ID. There is no evidence that the worker
claimed the lease or started execution. Complete the routing step and state the
highest justified ledger state.

## Case C — exhausted recovery without changed evidence

`INGEST-1` is `BLOCKED`, attempt 2 of 2. Its blocker fingerprint remains
`provider-429-us`; the latest evidence is byte-identical to the prior attempt.
Its predecessors are `ACCEPTED`, and no owner decision changed. Run Dispatcher
`RECOVER`.

## Case D — audited review failure with a dependent task

`API-1` is `REPORTED`. Sessions `RECEIVE_AUDIT` independently verifies the
expected worker, checkout, diff and `LEASE: RELEASED`; targeted tests pass, but
the required review proves an authorization bypass in the changed path.
`nobrainer-build` is the assigned repair method. `UI-1` depends on `API-1`.
No owner decision is needed. State the audit result, routing owner, invalidation
and successor state. Do not fix code.

## Case E — unresolved problem gate

A production retry mismatch is ambiguous. One relevant wiki lesson documents
three retries for last year's provider configuration. Current runtime evidence
shows five attempts. Current internet research is unavailable. Apply only the
shared `PROBLEM_GATE` and state whether any remedy may be selected.
