...
Resolve `BUG_HUNT_SKILL_DIR` to the absolute directory containing this
`SKILL.md`. The installed helper is `$BUG_HUNT_SKILL_DIR/scripts/bug-hunt` and
the template is `$BUG_HUNT_SKILL_DIR/assets/bug-template.md`; never resolve
either from the project working directory.
...
```bash
"$BUG_HUNT_SKILL_DIR/scripts/bug-hunt" --base origin/main --rounds 4
```
...
- Use the template at `$BUG_HUNT_SKILL_DIR/assets/bug-template.md`: title, severity, exact location, **why it's a real bug (proof, with the offending code)**, root cause, **proposed fix (diff/code)**, and a regression-guard test. Fill every section; no placeholders.
